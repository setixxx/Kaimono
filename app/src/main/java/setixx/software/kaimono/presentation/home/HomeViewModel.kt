...
import setixx.software.kaimono.domain.usecase.SearchProductsUseCase
...
class HomeViewModel @Inject constructor(
    ...
    fun onQueryChange(newQuery: String) {
        _state.update { it.copy(query = newQuery, page = 1, endReached = false, products = emptyList()) }
        searchJob?.cancel()
        searchJob = viewModelScope.launch {
            _state.update { it.copy(isLoading = true) }
            delay(500)
            _state.update { it.copy(page = 1, endReached = false, products = emptyList()) }
            getProducts()
        }
    }
